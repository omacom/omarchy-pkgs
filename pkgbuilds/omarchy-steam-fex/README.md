...
The launcher runs Steam through `muvm` and `FEXBash` with the CEF occlusion workaround. Once Steam's client files are present, it patches the Steam UI network initialization block that can leave login waiting indefinitely. Both initial and subsequent launches leave Steam's normal bootstrap, update, verification, and repair behavior enabled. If the FEX launcher is unavailable, it falls back to `steam`.

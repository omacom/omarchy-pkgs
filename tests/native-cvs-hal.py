...
verbatim from that checkout; no camera device is opened.
"""
import pathlib
import resource
import subprocess
...
source = (pathlib.Path(sys.argv[1]) / 'src/v4l2/MediaControl.cpp').read_text()
def method(name):
...
int main() {
    ...
    sensor.info.id = 1; sensor.info.type = MEDIA_ENT_F_CAM_SENSOR; sensor.info.links = 1;
